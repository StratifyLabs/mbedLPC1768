...
void debug_initialize() {
  ...
  const pio_attr_t attr = {.o_flags = PIO_FLAG_SET_OUTPUT,
                           .o_pinmask = (1 << CONFIG_DEBUG_LED_PIN)};
  sos_config.sys.pio_set_attributes(CONFIG_DEBUG_LED_PORT, &attr);
...
void debug_enable_led() {
  ...
  sos_config.sys.pio_write(CONFIG_DEBUG_LED_PORT, 1 << CONFIG_DEBUG_LED_PIN, 1);
}

void debug_disable_led() {
  // turn the debug LED off
  sos_config.sys.pio_write(CONFIG_DEBUG_LED_PORT, 1 << CONFIG_DEBUG_LED_PIN, 0);
}
