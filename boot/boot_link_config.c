...
link_transport_phy_t link_transport_open(const char *name,
  ...
  usb_attr.pin_assignment.dp = mcu_pin(CONFIG_USB_DP_PORT, CONFIG_USB_DP_PIN);
  usb_attr.pin_assignment.dm = mcu_pin(CONFIG_USB_DM_PORT, CONFIG_USB_DM_PIN);
  // usb_attr.freq = mcu_board_config.core_osc_freq;
  fd = boot_link_transport_usb_open(
      name, &m_usb_control, &sos_link_transport_usb_link_constants, &usb_handle,
      &usb_attr, mcu_pin(CONFIG_USB_PULLUP_PORT, CONFIG_USB_PULLUP_PIN),
      CONFIG_USB_PULLUP_ACTIVE_HIGH); // active high up pin
